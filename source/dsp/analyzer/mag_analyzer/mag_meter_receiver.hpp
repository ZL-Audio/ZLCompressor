...
namespace zldsp::analyzer {
    ...
    public:
        ...
        void run(const zldsp::container::FIFORange range,
                 std::vector<std::vector<float>>& sample_fifo,
                 const MagType mag_type) {
            dbs_.resize(sample_fifo.size());
            for (size_t chan = 0; chan < sample_fifo.size(); ++chan) {
                float value{0.f};
                ...
                if (mag_type == MagType::kPeak) {
                    dbs_[chan] = chore::gainToDecibels(value);
                } else {
                    dbs_[chan] = chore::squareGainToDecibels(
